# -*- coding: utf-8 -*-
"""
Created on Fri Jan 24 10:30:40 2020

@author: xueliang.chen
"""
...
def recommend(user, N):
    ...
    #统计每个用户打标签的总次数，以及用户打过标签的商品总数
    wut_sum = 0 #用户打标签总次数
    wti_sum = 0 #用户打过标签的商品总数
    ...
    for tag, wut in user_tags[user].items():
        ...
        for item, wti in tag_items[tag].items():
            ...
            if item not in recommend_items:
                recommend_items[item] = (wut/wut_sum)*(wti/wti_sum) 
            else:
                recommend_items[item] = recommend_items[item] + (wut/wut_sum)*(wti/wti_sum)
